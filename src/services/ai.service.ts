...
import { aiFeatureToggles as seedAiFeatureToggles } from '@/mocks'
...
let toggles: AiFeatureToggle[] = seedAiFeatureToggles.map((t) => ({ ...t }))

export const aiService = {
  /** Communication Center — draft a reply based on the patient's last message. */
  async suggestReply(conversation: Conversation, patientName: string): Promise<string> {
  ...
  async summarizeConversation(conversation: Conversation, patientName: string): Promise<string[]> {
  ...
  async generateTreatmentSummary(plan: TreatmentPlan, patient: Patient): Promise<string> {
  ...
  async draftPrescription(diagnosis: string): Promise<PrescriptionMedicine[]> {
  ...
  async generateReminderMessage(patientName: string, doctorName: string, date: string, time: string): Promise<string> {
    const first = firstName(patientName)
    const variants = [
  ...
  async generateFollowUpMessage(patientName: string, planTitle: string, nextStageLabel?: string): Promise<string> {
    const first = firstName(patientName)
    if (nextStageLabel) {
  ...
  async suggestAppointmentSlot(openSlots: string[]): Promise<string | null> {
  ...
  async answerKnowledgeQuestion(question: string): Promise<string> {
  ...
  async generatePatientAiSummary(
    patient: Patient,
  ...
  // --- AI feature settings (admin/ai-settings-page.tsx) ---
  async getSettings(): Promise<AiFeatureToggle[]> {
    return [...toggles]
  },
  async updateSetting(key: string, enabled: boolean): Promise<AiFeatureToggle> {
    toggles = toggles.map((t) => (t.key === key ? { ...t, enabled } : t))
